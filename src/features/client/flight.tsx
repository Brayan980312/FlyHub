...
import CompraAsientosForm from "./buySeatsFlights";

const BusquedaVuelos: React.FC = () => {
  const theme = useTheme();
  const { mostrarNotificacion } = useAppUI();

  /** Estado para mostrar formulario de compra de vuelos */
  ...
  /** Estados para compra de vuelos */
  const [vuelo, setVuelo] = useState<ResponseAllFlightAvailable | null>();
  const [pasajerosVuelo, setPasajerosVuelo] = useState(0);
  ...
  const handleBuySeat = (vuelo: ResponseAllFlightAvailable) => {
    setVuelo(vuelo);
    setPasajerosVuelo(Number(pasajeros));
    ...
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      {mostrarCompra && (
        <>
          <CompraAsientosForm
            vuelo={vuelo}
            pasajeros={pasajerosVuelo}
            cancelar={handleCancelarCompra}
          ></CompraAsientosForm>
        </>
      )}
    </Container>
